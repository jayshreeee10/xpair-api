function buildJSON(attributes, grouped = {}, attributeMap, withValidation = false) {
  ...
  function buildValidationBase(attrMeta, isMandatory, tableName, tableColumnName) {
    let enumValue = attrMeta?.enum;
    
    // Handle enum value - if it's a string, try to parse it as JSON
    if (typeof enumValue === 'string') {
      try {
        enumValue = JSON.parse(enumValue);
      } catch (e) {
        // If parsing fails, set to null/undefined so it won't be included
        enumValue = null;
      }
    }
    ...
    const base = {
      ...
      ...(Array.isArray(enumValue) && enumValue.length > 0 ? { enum: enumValue } : {})
    };
